...
async def handleAbortMessage(context, channel: ICommunicationChannel):
    ...
    context.client_abort = True # Keep state management with conn for now
    # 打断客户端说话状态
    # Use the channel interface to send the message
    await channel.send_json({"type": "tts", "state": "stop", "session_id": context.session_id})
    context.clearSpeakStatus() # Keep state management with conn for now
