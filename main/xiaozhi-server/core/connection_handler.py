...
class ConnectionHandler:
    ...
    async def chat_async(self, text):
        ...
        if self.channel:
            ...
            self.logger.bind(tag=TAG).error(
                "Channel not initialized in chat_async for send_stt_message"
            )
        ...
        try:
                    ...
                    if chunk.endswith((".", "。", "!", "！", "?", "？")):
                        ...
                        if text_to_speak:
                            ...
                            task = asyncio.create_task(
                                self.speak_and_play_async(
                                    text_to_speak, current_text_index
                                )
                            )
                        ...
                        llm_response_buffer = (
                            ""  # Clear buffer after processing segment
                        )
                ...
                async for completed_task in asyncio.as_completed(tts_tasks):
                    try:
                        ...
                        if result:
                            ...
                            self.dispatcher.dispatch_audio(
                                (opus_packets, response_text, text_index, "opus")
                            )
                            self.logger.bind(tag=TAG).info(
                            ...
                            self.logger.bind(tag=TAG).warning(
                                "TTS task completed but returned no result."
                            )
                    except Exception as e:
                        self.logger.bind(tag=TAG).error(
                            f"Error processing completed TTS task: {e}", exc_info=True
                        )

                # Save full response
                self.dialogue.put(
                    Message(role="assistant", content=full_assistant_response)
                )
            else:
                self.logger.bind(tag=TAG).warning(
                    "LLM returned empty stream or no processable text chunks."
                )
            ...
            self.logger.bind(tag=TAG).error(
                f"Error during LLM chat or TTS processing: {e}", exc_info=True
            )
            self.llm_finish_task = True
