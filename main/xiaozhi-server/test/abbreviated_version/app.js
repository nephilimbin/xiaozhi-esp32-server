  ...
  try {
    ...
    opusEncoder = {
      ...
      init: function () {
        ...
        // 初始化编码器
        const err = mod._opus_encoder_init(this.encoderPtr, this.sampleRate, this.channels, OPUS_APPLICATION);
      ...
      encode: function (pcmData) {
        ...
        // 编码
        const encodedBytes = mod._opus_encode(this.encoderPtr, pcmPtr, this.frameSize, encodedPtr, maxEncodedSize);
...
function stopRecording() {
  ...
  console.log('录制完成:', 'PCM帧数:', recordedPcmData.length, 'Opus帧数:', recordedOpusData.length);
}
...
function handleTextMessage(message) {
  if (message.type === 'hello') {
    ...
    if (message.state === 'start') {
      console.log(`服务器开始发送语音:${message.text}`);
      if (message.text) {
